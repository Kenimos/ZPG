...
void main()
{
    ...
    for (int i = 0; i < numLights; ++i)
    {
        ...
        if (diff > 0.0)
        {
            ...
            vec3 diffuseComponent = diff * light.color * light.intensity;

            vec3 reflectDir = reflect(-lightDir, norm);
            float spec = pow(max(dot(viewDir, reflectDir), 0.0), 32.0);
            vec3 specularComponent = specularStrength * spec * light.color * light.intensity;

            diffuse += diffuseComponent * attenuation;
